...
export type MerchantWebhookEvent = 'ticket.purchased' | 'event.sold_out' | 'payment.failed'
...
export async function deliverPendingWebhooks(limit = 20): Promise<number> {
  ...
  for (const job of jobs) {
    ...
    const signature = signMerchantPayload(
      (merchant?.webhook_secret as string | null) ?? 'bu-webhook',
      body,
    )
