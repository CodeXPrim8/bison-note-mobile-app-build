...
export type InvitationStatus = 'pending' | 'viewed' | 'accepted' | 'declined' | 'ticket_purchased'
...
export function publicTicketStatus(status: TicketStatus, eventEnd?: string | null): string {
  if (status === 'checked_in') return 'CHECKED IN'
  if (status === 'refunded') return 'REFUNDED'
  if (status === 'cancelled' || status === 'reserved') return status === 'cancelled' ? 'CANCELLED' : 'EXPIRED'
  if (eventEnd && new Date(eventEnd).getTime() < Date.now()) return 'EXPIRED'
