      ...
      .then(async (res) => {
        ...
        setEvents(
          list.map((event) => ({
            ...
            date: event.start_time ? new Date(String(event.start_time)).toLocaleString() : '',
      ...
      .then(async (res) => {
        ...
        setRecentTransfers(
          txs.slice(0, 8).map((tx) => ({
            ...
            fromGuest: 'Guest',
            timestamp: tx.created_at ? new Date(String(tx.created_at)).toLocaleString() : '',
          })),
        )
      })
      .catch(() => undefined)
  ...
  return (
    <div className="space-y-6 pb-24 pt-4">
      ...
      <div className="space-y-4 bg-gradient-to-b from-primary to-primary/80 px-4 py-8 text-primary-foreground">
        ...
        <div className="space-y-3 rounded-2xl bg-primary-foreground/10 p-4 backdrop-blur">
          ...
          <div className="flex items-center justify-between">
            <div className="text-3xl font-bold">Ƀ {totalBUReceived.toLocaleString()}</div>
            <div className="rounded-full bg-primary-foreground/20 p-2">
              <TrendingUp size={20} />
            </div>
          </div>
          <div className="text-sm opacity-90">
            ≈ ₦{totalBUReceived.toLocaleString()}
          </div>
        </div>
      </div>
      ...
      <div className="px-4">
        ...
        <div className="space-y-3">
          {recentTransfers.length === 0 && <p className="text-sm text-muted-foreground">No ɃU received yet.</p>}
